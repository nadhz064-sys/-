...
if (!isset($_SESSION['user_id'])) {
    echo "يجب تسجيل الدخول كمستخدم للوصول إلى هذه الصفحة!";
    exit();
}

$user_id = $_SESSION['user_id']; // تخزين رقم المستخدم المسجل

// جلب البلاغات التي قدمها هذا المستخدم فقط
$sql = "SELECT id, address, time, date, status FROM reports WHERE user_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();

// جلب عدد البلاغات الكلي الخاصة بهذا المستخدم
$sql_total = "SELECT COUNT(*) as total FROM reports WHERE user_id = ?";
...
// جلب عدد البلاغات قيد المعالجة الخاصة بهذا المستخدم
$sql_inProgress = "SELECT COUNT(*) as in_progress FROM reports WHERE user_id = ? AND status = 'قيد المعالجة'";
...
<div class="table-container">
    ...
    <tbody>
    <?php
$stmt = $conn->prepare("SELECT id, address, date, time, status, photo FROM reports WHERE user_id = ? ORDER BY date DESC");
$stmt->bind_param("i", $user_id);
